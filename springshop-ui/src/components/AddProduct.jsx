...
const AddProduct = () => {
  ...
  const generateDescription = async () => {
    ...
    // Clear existing description before streaming
    setProduct(prev => ({ ...prev, description: "" }));

    try {
      const response = await fetch(
        `${baseUrl}/api/product/stream-description?name=${encodeURIComponent(product.name)}&category=${encodeURIComponent(product.category)}`,
        { method: "POST" }
      );
      ...
      while (true) {
        ...
        setProduct(prev => ({ ...prev, description: accumulated }));
      }
